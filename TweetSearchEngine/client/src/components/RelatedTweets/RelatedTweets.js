...
function RelatedTweets(props) {
    ...
    var topicsString = props.namedEntities.map((entity) => entity.text).join(" ");
    const [relatedTweets, setRelatedTweets] = useState([]);

    useEffect(() => {
        axios.get(`http://${host}:${port}/advancedSearch/%20/${topicsString}/${props.location}/%20/${startDate}/${endDate}`).then((res) => {
